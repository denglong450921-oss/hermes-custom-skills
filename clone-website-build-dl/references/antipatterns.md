...
| 15 | Freezing animation before recording runtime states | GSAP, ScrollMagic, and reveal layers can disappear from the reference, producing false blank regions and a static clone | Run `$CLONE_EXTRACT_DIR/scripts/audit-animations.mjs` first. Record triggers and start/mid/end styles, then freeze for deterministic static QA |
| 16 | Recording approximate section heights without a spacing graph | Headings, media, and CTAs drift even when the total page height looks plausible | Run `$CLONE_EXTRACT_DIR/scripts/audit-spacing.mjs`. Record landmark rectangles, sibling gaps, edge insets, breakpoint deltas, and intentional whitespace boundaries |
