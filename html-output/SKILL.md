...
| Skill load fails with `Failed to load skill for /&lt;skill-name&gt;` (any skill slash command fails to load) | Check for duplicate SKILL.md files sharing the same `name:` YAML frontmatter across subdirectories. Darwin skill-optimizer creates `*-workspace/skill-snapshot/SKILL.md` copies that register with the same skill name, silently overriding the real one. Delete the conflicting SKILL.md. | Reload skills via `hermes gateway run --replace`, or wait for the next slash-command invocation to trigger a re-scan. |
...
| `references/python-html-pitfalls.md` | Python-specific pitfalls when generating HTML files (e.g. `\n` vs `\\n` escaping) |
| `references/web-content-sourcing.md` | Extracting content from Chinese platforms (WeChat, etc.) behind anti-bot walls — curl UA spoofing + Python regex extraction |
